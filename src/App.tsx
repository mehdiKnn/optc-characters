...
import { addMergedCondition, candidatePool, candidateProgress, conditionState, hasFamilyConflict, isUnrestrictedFriendSelection, matchesModifier, mergeConditions, rumbleConditionsToComposition, supportVerdict, teamCost, type MergedCondition } from './domain/engine'
...
function TeamBuilder({ team, boxIds, cap, onBack, onChange }: { team: Team; boxIds: Id[]; cap: number; onBack: () => void; onChange: (team: Team) => void }) {
  ...
  const unrestrictedFriendSelection = isUnrestrictedFriendSelection(team.type, activeSlot, supportFor !== null)
  ...
  const availableCandidates = candidatePool(team.type, activeSlot, supportFor !== null, boxIds, data.units)
  const scored = availableCandidates.filter(unit => !query || `${unit.id} ${unit.name}`.toLowerCase().includes(query.toLowerCase())).filter(unit => !type || unit.types.includes(type)).filter(unit => !unitClass || unit.classes.includes(unitClass)).filter(unit => !tag || unit.tags.includes(tag)).map(unit => ({ unit, ...candidateProgress(unit, crew, checked) })).filter(item => !item.violates && (!checked.some(condition => !condition.negative && !conditionState(condition, crew).done) || item.score > 0)).sort((a, b) => b.score - a.score || byDescendingId(a.unit, b.unit))
